...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const displayPosts = (posts) => { DOMElements.postsContainer.innerHTML = ''; if(posts.length === 0) { DOMElements.postsContainer.innerHTML = `<div class="post" style="text-align:center;">No posts yet.</div>`; return; } posts.forEach(p => DOMElements.postsContainer.appendChild(window.createPostElement(p))); };
