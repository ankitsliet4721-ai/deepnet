...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const firebaseConfig = {
    ...
    storageBucket: "deep-9656b.appspot.com",
  ...
  const DOMElements = {
    ...
    profilePicModal: document.getElementById('profilePicModal'),
    profilePicUploadTrigger: document.getElementById('profilePicUploadTrigger'),
    profilePicInput: document.getElementById('profilePicInput'),
    profilePicPreview: document.getElementById('profilePicPreview'),
    profilePicCancel: document.getElementById('profilePicCancel'),
    profilePicSave: document.getElementById('profilePicSave'),
  ...
  const handleProfilePicSelect = (event) => { const file = event.target.files[0]; if (!file) return; if (!file.type.startsWith('image/')) return showToast('Please select an image file.', 'error'); if (file.size > 5 * 1024 * 1024) return showToast('File is too large. Max 5MB.', 'error'); selectedProfilePicFile = file; const reader = new FileReader(); reader.onload = (e) => { DOMElements.profilePicPreview.src = e.target.result; }; reader.readAsDataURL(file); DOMElements.profilePicModal.classList.remove('hidden'); };
  const uploadProfilePic = async () => { if (!selectedProfilePicFile || !currentUser) return; DOMElements.uploadSpinner.classList.remove('hidden'); DOMElements.profilePicSave.disabled = true; try { const filePath = `profile_pictures/${currentUser.uid}`; const storageRef = ref(storage, filePath); const snapshot = await uploadBytes(storageRef, selectedProfilePicFile); const downloadURL = await getDownloadURL(snapshot.ref); await updateProfile(auth.currentUser, { photoURL: downloadURL }); await updateDoc(doc(db, 'users', currentUser.uid), { photoURL: downloadURL }); currentUser.photoURL = downloadURL; [DOMElements.userAvatar, DOMElements.sidebarAvatar, DOMElements.composerAvatar].forEach(el => { if (el) el.src = downloadURL; }); showToast('Profile picture updated!', 'success'); closeProfilePicModal(); } catch (error) { console.error("Upload Error:", error); showToast('Upload failed.', 'error'); } finally { DOMElements.uploadSpinner.classList.add('hidden'); DOMElements.profilePicSave.disabled = false; } };
  const closeProfilePicModal = () => { DOMElements.profilePicModal.classList.add('hidden'); DOMElements.profilePicInput.value = ''; selectedProfilePicFile = null; };
  ...
  const listenForNotifications = () => { if (notificationsListener) notificationsListener(); const q = query(collection(db, 'notifications'), where('recipientId', '==', currentUser.uid), orderBy('createdAt', 'desc'), limit(10)); let isFirstLoad = true; notificationsListener = onSnapshot(q, s => { const notifs = s.docs.map(d => ({ id: d.id, ...d.data() })); const unread = notifs.filter(n => !n.read).length; DOMElements.notificationCount.textContent = unread; DOMElements.notificationCount.classList.toggle('hidden', unread === 0); if (!isFirstLoad && unread > 0) { DOMElements.notificationsToggle.classList.add('shake'); setTimeout(() => DOMElements.notificationsToggle.classList.remove('shake'), 500); } renderNotifications(notifs); isFirstLoad = false; }); };
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user && user.emailVerified) {
      ...
      DOMElements.app.classList.remove('hidden'); DOMElements.loginModal.classList.add('hidden');
      ...
      DOMElements.profilePicPreview.src = currentUser.photoURL;
      ...
      currentUser=null; DOMElements.app.classList.add('hidden'); DOMElements.loginModal.classList.remove('hidden');
      if(postsListener)postsListener(); if(usersListener)usersListener(); if(notificationsListener)notificationsListener();
    }
  });
  ...
  DOMElements.profilePicUploadTrigger.addEventListener('click', () => DOMElements.profilePicInput.click());
  DOMElements.profilePicInput.addEventListener('change', handleProfilePicSelect);
  DOMElements.profilePicCancel.addEventListener('click', closeProfilePicModal);
  DOMElements.profilePicSave.addEventListener('click', uploadProfilePic);
